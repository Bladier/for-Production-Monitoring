...
#Region "Procedures and Functions"
    ...
    Public Function LoginUser(ByVal user As String, ByVal password As String) As Boolean
        ...
        mySql &= vbCrLf & String.Format(" WHERE LOWER(Username) = LOWER('{0}') AND Password = '{1}' AND STATUS <> '0'", user, Encrypt(password))
        Dim ds As DataSet
    ...
    Public Sub ChangePassword()
        Dim mySql As String = "SELECT * FROM tbl_Gamit WHERE ID = " & POSuser.UserID
        Dim ds As DataSet = LoadSQL(mySql, fillData)

        ds.Tables(fillData).Rows(0).Item("Password") = Encrypt(_password)
        SaveEntry(ds, False)
    End Sub

    Public Sub DeleteUser(ByVal Status As Boolean)
        Dim mySql As String = "SELECT * FROM tbl_Gamit WHERE ID = " & _userID
        Dim ds As DataSet = LoadSQL(mySql, fillData)
        If Status = True Then
